	...
	public partial class CitiesCSCController : CSCBaseApiController
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCSC.City updatedDBItem, ref IRepositoryActionResult<entCSC.City> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = RunSync<IRepositoryActionResult<entCSC.City>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCSC.City>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCSC.City>(() => Repo.Get_CityAsync(cityId, 1));
