	...
	public partial class CountriesCSCController : CSCBaseApiController
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCSC.Country updatedDBItem, ref IRepositoryActionResult<entCSC.Country> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = RunSync<IRepositoryActionResult<entCSC.Country>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCSC.Country>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCSC.Country>(() => Repo.Get_CountryAsync(countryId, 1));
